...
class ReflectionTest {
    ...
    void givenObject_whenGetsFieldNamesAtRuntime_thenCorrect() {
        ...
        final Field[] fields = student.getClass().getDeclaredFields(); // 값이 할당되지 않고 선언만 된 변수까지 가져오는 메서드
    ...
    void givenClass_whenGetsFieldsByName_thenCorrect() throws Exception {
        final Class<?> questionClass = Question.class;
        final Field field = questionClass.getDeclaredField("questionId"); // getField는 public field만 가져온다.
    ...
    void givenClassField_whenSetsAndGetsValue_thenCorrect() throws Exception {
        ...
        Arrays.stream(studentClass.getFields()) // public 필드를 로그에 찍어보면
                .forEach(method -> log.info(method.getName())); // 아무것도 나오지 않는다.
    }
